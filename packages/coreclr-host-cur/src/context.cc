...
extern "C" typedef DotNetHandle* (*ManagedFunction)(
    int argc, const char* argv[],
    int jsArgC, JsHandle** jsArgV,
    DotNetHandle* resultValue
    );
...
Napi::Value Context::RunCoreApp(const Napi::CallbackInfo& info) {
  ...
  auto entry_point_ptr = context->host_->GetManagedFunction(
      "CoreclrNodeHost.NativeHost.NativeEntryPoint, CoreclrNodeHost",
      "RunHostedApplication",
      "CoreclrNodeHost.NativeHost.EntryPointSignature, "
      "CoreclrNodeHost");
...
Napi::Value Context::CallManagedFunction(const Napi::CallbackInfo &info) {
  auto context = Context::current_;
  if (Context::current_ == nullptr) return Napi::Value();
  ThreadInstance _(Context::current_);

  Napi::Env env = info.Env();
  if (info.Length() < 2){
    Napi::TypeError::New(env, "Found wrong number of arguments, expected three arguments")
        .ThrowAsJavaScriptException();
  }

  std::vector<std::string> arguments(2);
  std::vector<const char*> arguments_c(2);
  for (auto i = 0u; i < 2; i++) {
    if (!info[i].IsString()) {
      Napi::Error::New(env, "Expected only string arguments")
          .ThrowAsJavaScriptException();
  ...
  std::vector<JsHandle> jsArguments(info.Length() -2);
  std::vector<JsHandle*> jsArgV(0);
  for (auto i=0u; i< info.Length() -2; i++){
    jsArguments[i] = JsHandle::FromValue(info[i+2]);
    jsArgV.push_back(&jsArguments[i]);
  }
  auto the_ptr = context->host_->GetManagedFunction(
      "CoreclrNodeHost.NativeHost.NativeEntryPoint, CoreclrNodeHost",
      "InvokeManagedFunction",
      "CoreclrNodeHost.NativeHost.ManagedFunctionSignature, CoreclrNodeHost"
  );
  auto managed_ptr = reinterpret_cast<ManagedFunction>(the_ptr);
  DotNetHandle return_handle;
  managed_ptr(
      arguments_c.size(), arguments_c.data(),
      jsArgV.size(), jsArgV.data(),
      &return_handle
  );
  return return_handle.ToValue(env, context->function_factory_, context->array_buffer_factory_);
}
